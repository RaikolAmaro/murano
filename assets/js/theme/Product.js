...
export default class Product {
  ...
  _initTabs() {
    ...
    $(window).on('resize', debounce(() => {
      console.log('resized');
      this._tabsHeight($('[data-tab-content]'));
    }, 300));
  }
    ...
    imagesLoaded($content[0], () => {
      ...
      if (window.innerWidth >= this.mobileWidth) {
        $('.product-tabs-wrapper')
          .css('height', tabsHeight)
          .removeClass('product-tabs-wrapper-hidden')
        ;
      } else {
        $('.product-tabs-wrapper')
          .css('height', '')
  ...
  _bindEvents() {
    ...
    $('[data-reviews-link]').on('click', (event) => {
      ...
      this.scrollLink.scrollToContent('#write_review');
